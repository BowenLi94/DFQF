...
#-------------------------------------------------------------------------
def distiller_quantize(x, num_bits, alpha, half_range):
    if half_range:
        min_q_val, max_q_val = get_quantized_range(num_bits, signed=False)
        scale = asymmetric_linear_quantization_scale_factor(num_bits, 0, alpha)
    else:
...
#-------------------------------------------------------------------------
def mse_histogram_clip(bin_x, bin_y, num_bits, alpha, half_range):
   ...
   idx = np.abs(bin_x) <= alpha
   bin_xq = distiller_quantize(bin_x[idx], num_bits, alpha, half_range)
...
# Minimal MSE
#-------------------------------------------------------------------------
def find_clip_mmse(values, num_bits, half_range= False):
    # Build histogram
    if half_range:
        values[values<0] = 0 # ReLU
    ...
    mses = [ mse_histogram_clip(bin_x, bin_y, num_bits, alpha, half_range)
             for alpha in alphas ]

    alpha_best = alphas[np.argmin(mses)]
    #print(" MSE alpha_best = %5.2f / %5.2f(%5.2f)" % (alpha_best, max_abs,alpha_best/max_abs))
    return alpha_best
...
#alpha_gauss_positive = {1:1.47818312, 2:1.80489289, 3:2.19227856, 4:2.57733584, 5:2.94451183, 6:3.29076248, 7:3.61691335, 8:}
#alpha_laplace =          {2:2.33152939, 3:3.04528770, 4:4.00378631, 5:5.08252088, 6:6.23211675, 7:7.42700429, 8:8.65265030}
#alpha_laplace_positive = {1:2.33152939, 2:3.04528770, 3:4.00378631, 4:5.08252088, 5:6.23211675, 6:7.42700429, 7:8.65265030, 8:}
...
def find_clip_aciq(values, num_bits, half_range = False):
    # Gaussian clip
    ...
    alpha_g = alpha_gauss_positive[num_bits] * sigma if half_range else alpha_gauss[num_bits] * sigma
    # Laplacian clip
    b = np.mean(np.abs(values - np.mean(values)))
    alpha_l = alpha_laplace_positive[num_bits] * b if half_range else alpha_laplace[num_bits] * b 
    ...
    mse_gauss = mse_histogram_clip(bin_x, bin_y, num_bits, alpha_g, half_range)
    mse_laplace = mse_histogram_clip(bin_x, bin_y, num_bits, alpha_l, half_range)

    alpha_best = alpha_g if mse_gauss < mse_laplace else alpha_l
    #print(" ACIQ alpha_best = %7.4f / %7.4f(%7.4f)" % (alpha_best, max_abs,alpha_best/max_abs))
    return alpha_best
...
#-------------------------------------------------------------------------
def find_clip_entropy(values, num_bits, half_range = False):
    if half_range:
        values[values<0] = 0
    max_abs = np.max(np.abs(values))
    ...
    alpha_best = opt_th
    #print(" Entropy alpha_best = %5.2f / %5.2f(%5.2f)" % (alpha_best, max_abs, alpha_best/max_abs))
    return alpha_best
